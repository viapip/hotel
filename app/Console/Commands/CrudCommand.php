...
class CrudCommand extends Command
{
    ...
    protected $file_field =  'resources/CrudJson.json';
    ...
    public function handle()
    {
        ...
        if ($this->file_field) {
            $fields = $this->processJSONFields($this->file_field);
        }
        ...
        if ($this->file_field) {
            $foreignKeys = $this->processJSONForeignKeys($this->file_field);
        }

        $validations = trim($this->option('validations'));
        if ($this->file_field) {
            $validations = $this->processJSONValidations($this->file_field);
        }
        ...
        if ($this->file_field) {
            $relationships = $this->processJSONRelationships($this->file_field);
        }
