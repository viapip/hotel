                            ...
                            <table class="table">
                                ...
                                @foreach($contact as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->title }}</td><td>{{ $item->image }}</td><td>{{ $item->name }}</td>
                                        <td>
                                            ...
                                            <a href="{{ url('/admin/contact/' . $item->id . '/edit') }}" title="Редактировать Contact"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Редактировать</button></a>

                                            <form method="POST" action="{{ url('/admin/contact' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm" title="Улдалить Contact" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Удалить</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
