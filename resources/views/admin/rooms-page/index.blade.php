                            ...
                            <table class="table">
                                ...
                                @foreach($roomspage as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->title }}</td><td>{{ $item->banner }}</td><td>{{ $item->title_seo }}</td>
                                        <td>
                                            ...
                                            <a href="{{ url('/admin/rooms-page/' . $item->id . '/edit') }}" title="Редактировать RoomsPage"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Редактировать</button></a>

                                            <form method="POST" action="{{ url('/admin/rooms-page' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                @method('DELETE')
                                                @csrf
                                                <button type="submit" class="btn btn-danger btn-sm" title="Улдалить RoomsPage" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Удалить</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
